...
public class TAB {
    ...
    private static final HashMap<UUID, TAB> lookUpTable = new HashMap<>();
     ...
     */
    public static TAB getPlayerList(Player player) {
        if (!lookUpTable.containsKey(player.getUniqueId()))
            return new TAB(player, SIZE_TWO);
        return lookUpTable.get(player.getUniqueId());
    }

    public TAB(Player player, int size) {
    ...
    private static void error() {
        Bukkit.broadcastMessage("ngu thi chet");
    }
}
...
class Skin implements ConfigurationSerializable {
    ...
    public static void getSkin(String username, SkinCallBack callBack) {
        ...
        if (newcall) {
            ...
            }.runTaskAsynchronously(TAB.plugin);
        }
    }
